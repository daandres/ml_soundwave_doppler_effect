...
Die Idee des evolutionären Traiinigs ist wie neuronale Netze biologisch
...
dabei, dass eine geeignetete Repräsentation des Netzwerkes durch ein Genom
...
\end{tabular} 
\caption[Tests für Rauschentfernung]{Tests um den Einfluss von Rauschentfernung zu untersuchen. Das Trainigsverfahren ist \texttt{RProp}. Die Anzahl der LSTM-Blöcke ist 16. Iterationen 100.}
...
möglich. In den eriteren Kapiteln wird auf die unterschiedlichen Parameter noch
...
\texttt{IClassifier}. Die Implementatierung des \ac{LSTM}-Klassifikators ist in
...
\item \textit{LSTMTrain}: Trainiert ein mit verschiednen Algorithmen das LSTM-Netz.
\end{itemize}
...
vesucht. Bei dieser Implementaion muss der Auschlag einen bestimmten Schwellwert 
...
Idee, es werden aber nicht ab dem Beginn des Auschlags die Daten an das 
...
bestätigt, welche zeigt, dass Backpropagtaion schwankende Ergebnisse liefert
...
Zwischenzeiten nach jedem Zehntel der Trainngsiterationen berechnet. In
\autoref{tab:inputtests} sind zusätzich zu den tests auch noch
Durchschnittswerte von Zeiten enthalten. Die beiden Durchschnittszeiten von
Minuten pro Iteration beziehen sich einersteits auf die Vorberechnung der Daten,
zum Anderen ist eine Differenz durch minimal unterschiedliche Taktfrequenzen der
Prozessoren beim Training zu erkären.
...
zehn \ac{LSTM}-Blöcken (TGewichte für die EIngaben und jedes Gate) und sieben
...
Insgesamt erkennt das Neuronale-Netz 13,77\% der Gesten niht korrekt. 
