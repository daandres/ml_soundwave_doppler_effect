...
nachgebildet in dem jeder Eingabewert bei der Summierung individuell mit $w_i$ gewichtet
wird. Die Gewichte entsprechen den unterschiedlich stark ausgeprägten Synapsen.
...
\label{fig:lstm_netz}
\end{figure}
Die genaue Anzahl der Neuronen für das Input- und Hidden-Layer stand zu Beginn des Projektes nicht fest. 
Sie wurde durch Tests, die in Kapitel \autoref{sec:lstm_data} beschrieben sind, ermittelt. 
Die Anzahl der Neuronen in dem Output-Layer ergab sich aus der Anzahl der Gesten. Es ist 
üblich für jedes Ergebnis ein eigenes Output-Neuron zu erstellen.
