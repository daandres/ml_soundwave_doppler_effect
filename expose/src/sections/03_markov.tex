...
Verwendet werden Objekte der Klassen \testtt{sklearn.hmm.GMMHMM} und \texttt{sklearn.mixture.GMM} aus \texttt{scikit-learn}-Bibliothek \url{scikit-learn.org}.
...
wurden die \testtt{sklearn}-Klassen erweitert, so entstehen die Klassen \testtt{GestureHMM} und \testtt{GestureGMM}. Dies ist notwendig, da die fertig trainierten
\acl{GMM}s während des \acl{HMM} Trainings noch einmal verändert werden - was allerdings unerwünscht ist. 

\begin{figure}[htbp] \centering
    \includegraphics[width=0.6\textwidth]{markov/gmm_data.png}
