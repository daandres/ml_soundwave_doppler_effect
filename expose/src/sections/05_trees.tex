...
%\begin{align*}
%sl &= \text{Anzahl Linksverschiebungen} \\
%sr &= \text{Anzahl Rechtsverschiebungen} \\
%s &= \text{Anzahl aller Verschiebungen} \\
%o &= \text{Reihenfolge der Verschiebungen} \\
%c &= \text{Anzahl der gleichzeitigen Verschiebungen} \\
%a &= \text{Differenz der Amplituden} \\
%dc &= \text{Mittel der Abstände zwischen R-L- oder L-R-Verschiebungen} \\
%de &= \text{Mittel der Abstände zwischen R-R- oder L-L-Verschiebungen}
%\end{align*}

Anzahl der linken Frequenzverschiebungen

Für die Klassifikation wurde auf Klassen und Methoden des Moduls \textit{sklearn.ensemble}~\cite{sklearn.ensemble} der Bibliothek \textit{sklearn}~\cite{sklearn} zurückgegriffen.



\subsection{Evaluation}
Während der Trainingsphase wurden aus der Bibliothek~\textit{sklearn} drei verschiedene Ensemble-Verfahren evaluiert, mit dem RandomForestClassifier ein Bagging-Verfahren und mit AdaBoost und dem GradientBoostingClassifier zwei Boosting-Verfahren. 
Anhand der extrahierten Merkmale der Gesten wurden dabei mit dem GradientBoostingClassifier die besten Ergebnisse erzielt. 
Bei der Klassifikation aller Gesten lag die Erfolgsrate bei ca. 62\%, wobei das beste Ergebnis mit 23 Schätzern und einer Lernrate von 0,2 erreicht wurde. 
...
Ohne die Geste \acl{RLO} konnte die Erfolgsrate auf 75\% erhöht werden.
Das Ergebnis lässt sich dadurch erklären, dass sich die beiden Gesten \acl{RLO} und \acl{SPO} in ihrer Charakteristik sehr ähnlich sind. 
Auch unter Berücksichtigung der Amplitudenwerte als zusätzliches Entscheidungskriterium konnte die Erfolgsrate bei der Klassifikation aller Gesten nicht wesentlich verbessert werden. Der Vergleich der Erfolgsrate des GradientBoostingClassifiers bei 4 und 5 Gesten ist in Abbildung~\ref{fig:vgl_gradientboosting} dargestellt. Bei beiden Tests wurde die Lernrate auf 0,2 und die maximale Baumtiefe auf 2 gesetzt, da mit dieser Konfiguration im Schnitt die besten Ergebnisse erzielt werden konnten. 
