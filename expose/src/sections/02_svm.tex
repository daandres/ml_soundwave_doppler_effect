...
\begin{figure}[htbp] \centering
    \includegraphics[width=0.25\textwidth]{svm/svm_xor.png}
...
Die dritte Reduzierung wird mittels der Addition benachbarter Frames erzielt, so dass letztlich nur noch jeweils 10 Frames zu je 40 Werten pro Datensatz/Geste existieren.
...
\begin{figure*}[htbp] \centering
    \includegraphics[width=0.8\textwidth]{svm/svm_preprocessing_steps.png}
    \caption{Vorverarbeitungsschritte}
    \label{fig:svm_preprocessing_steps}
\end{figure*}


Die Klassifikatoren erhalten dabei jeweils einzelne Frames zu je 64 Werten, die sich innerhab des Frequenzbereichs um die Referenzfrequenz von $18\text{,}5\text{ kHz}$ befinden.
